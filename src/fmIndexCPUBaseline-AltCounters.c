...
#include "omp.h"
  ...
  #define IN_INDEX_TAG      201
#else
  #define IN_INDEX_TAG      200
#endif
...
void searchIndexCPU(void *index, void *dataqueries, void *resIntervals)
{
  ...
  for (cntquery = 0; cntquery < numqueries; cntquery++) {
    ...
    for(j = iterations; j >= 0; j-=K_STEPS) {
      ...
      idxEntryL = 0; idxEntryR = 0;
      if ( (((indexCounterL % 2) != 0) && (indexBase <  NUM_COUNTERS_SLIM)) ||
         (((indexCounterL % 2) == 0) && (indexBase >= NUM_COUNTERS_SLIM)))
        idxEntryL = 1;

      if ( (((indexCounterR % 2) != 0) && (indexBase <  NUM_COUNTERS_SLIM)) ||
         (((indexCounterR % 2) == 0) && (indexBase >= NUM_COUNTERS_SLIM)))
        idxEntryR = 1;


      bcountL = indexFM[indexCounterL + idxEntryL].cnt[indexBase & (NUM_COUNTERS_SLIM - 1)];
      bcountR = indexFM[indexCounterR + idxEntryR].cnt[indexBase & (NUM_COUNTERS_SLIM - 1)];
      ...
      for(n = 0; n < NUM_BITMAPS; n++){
        ...
        bitmap = (idxEntryL) ? ~bitmap : bitmap;
      ...
      if (idxEntryL) L = bcountL - bitCount;
        else L = bcountL + bitCount;
      ...
      for(n = 0; n < NUM_BITMAPS; n++){
        ...
        bitmap = (shift > 32) ? mask : bitmapShifted;
        bitmap = (shift > 0) ? bitmap : 0x0;
        bitmap = (idxEntryR) ? ~bitmap : bitmap;
      ...
      if (idxEntryR) R = bcountR - bitCount;
        else R = bcountR + bitCount;
    }
