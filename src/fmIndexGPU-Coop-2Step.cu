...
#define SIZE_ENTRY_TO_WARP  (TOTAL_NUM_BITMAPS + (NUM_COUNTERS / 2))
#define THREADS_PER_REQUEST (SIZE_ENTRY_TO_WARP / SIZE_VECTOR_TYPE)
...
inline __device__ uint32_t computeBitmaps(uint4 bitmap, uint32_t interval, uint32_t indexBase, uint32_t localRequestThreadIdx)
{
  ...
  int32_t shift = (interval % NUM_CHUNK) - (localRequestThreadIdx * 32);
    ...
    for(queryWord = 0; queryWord < sizeQuery/4; queryWord++) {
      ...
      for(int32_t j = 0; j < 4/K_STEPS; j++) {
        ...
        for(int32_t i = 0; i < NUM_LOADS; i++){
          ...
          auxIndexData = (THREADS_PER_REQUEST - 2) + (sharedIndexBase >> 2);
          indexData = (localRequestThreadIdx > (THREADS_PER_REQUEST - 3)) ? auxIndexData : indexData;
          ...
          if(localRequestThreadIdx < (THREADS_PER_REQUEST - 1))
            result = computeBitmaps(loadData, sharedInterval, sharedIndexBase, localRequestThreadIdx);
          else
            result = selectCounter(loadData, sharedIndexBase & 0x3);

          result = (localRequestThreadIdx == (THREADS_PER_REQUEST - 2)) ? 0 : result;
          result = reduceEntry(result, localThreadIdx);
        ...
        for(int32_t s = 0; s < K_STEPS; s++)
          if(modposdollarBWT[s] == (interval / NUM_CHUNK))
            aux_interval = ((indexBase == dollarBaseBWT[s]) && (interval > dollarPositionBWT[s])) ? aux_interval-1 : aux_interval;
