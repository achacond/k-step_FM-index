...
#define SIZE_ENTRY          (((BITS_PER_BASE * NUM_BITMAPS * K_STEPS) + NUM_COUNTERS_SLIM) / SIZE_VECTOR_TYPE)

typedef struct {
  uint4 entry[SIZE_ENTRY];
} bitcnt_t;
...
inline __device__ uint32_t countBitmap(uint32_t bitmap, int32_t shift, uint32_t idxEntry)
{
  uint32_t mask;
  mask = 0xFFFFFFFF << (32 - shift);
  mask = (shift > 32) ? 0xFFFFFFFF : mask;
  mask = (shift > 0) ? mask : 0x0;
  mask = (idxEntry) ? ~mask : mask;
  return (__popc(bitmap & mask));
}

inline __device__ uint32_t computeBitmaps(uint4 bitmap, int32_t shift, uint32_t indexBase, uint32_t idxEntry)
{
  uint32_t bitmapA;
  ...
  bitmapA = countBitmap(bitmapA, shift, idxEntry);
                                  ...
                                  uint32_t numQueries, uint32_t sizeQuery, char *queries, uint32_t *results)
{
  ...
  uint32_t indexBase, idxEntry, flg2, flg3, valueCounter;
  ...
  if ((threadIdx.x < MAX_THREADS_PER_SM) && (globalThreadIdx < (numQueries * 2))){
    ...
    for(queryWord = 0; queryWord < nQueryWords; queryWord++) {
      ...
      for(j = 0; j < 4/K_STEPS; j++) {
        ...
        idxEntry = ((((indexCounter % 2) != 0) && (indexBase <  NUM_COUNTERS_SLIM)) || (((indexCounter % 2) == 0) && (indexBase >= NUM_COUNTERS_SLIM))) ? 1 : 0;
        bitCount = 0;
        for(n = 0; n < NUM_BITMAPS; n++){
          #if defined(LDG)
            buffer = __ldg(&(indexFM[indexCounter].entry[(NUM_COUNTERS_SLIM / SIZE_VECTOR_TYPE) + n]));
          #else
            buffer = indexFM[indexCounter].entry[(NUM_COUNTERS_SLIM / SIZE_VECTOR_TYPE) + n];
          #endif
            bitCount += computeBitmaps(buffer, shift, indexBase, idxEntry);
            shift -= 32;
        }
        ...
        #if defined(LDG)
          buffer = __ldg(&(indexFM[indexCounter + idxEntry].entry[(indexBase >> 2) & 0x1]));
        #else
          buffer = indexFM[indexCounter + idxEntry].entry[(indexBase >> 2) & 0x1];
        #endif
        valueCounter = selectCounter(buffer, indexBase & 0x3);
        interval = (idxEntry) ? valueCounter - bitCount : valueCounter + bitCount;
      }
    }
